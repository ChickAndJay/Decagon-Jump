...
#define FRAGMENT_SETUP(x) FragmentCommonData x = \
	FragmentSetup(i.tex, i.eyeVec.xyz, IN_VIEWDIR4PARALLAX(i), i.tangentToWorldAndParallax, IN_WORLDPOS(i));
...
half4 fragForwardBaseInternal (VertexOutputForwardBase i)
{
	...
	//Polybox Calculate heightFog
 	...
 	half heightFog = saturate((_HeightFogEnd -  i.posWorld.y) / (_HeightFogEnd-_HeightFogStart));
...
half4 fragForwardAddInternal (VertexOutputForwardAdd i)
{
	...
	UNITY_LIGHT_ATTENUATION(atten, i, s.posWorld)
	half q = atten;
    UnityLight light = AdditiveLight (IN_LIGHTDIR_FWDADD(i), q);
	...
	UNITY_EXTRACT_FOG_FROM_EYE_VEC(i);
	//Polybox Calculate heightFog
	half3 heightFog = saturate((_HeightFogEnd -  i.posWorld.y) / (_HeightFogEnd-_HeightFogStart));
	heightFog = pow(heightFog,2.2);
